...
if uploaded_file:
    ...
    # ================== Nome Excel ==================
    original_name = st.session_state.get("uploaded_filename","fatura")
    ...
    if st.button("Enviar total para SharePoint"):
        ...
        df_all = pd.concat([
            st.session_state.df_transacoes.rename(columns={"Estabelecimento":"Descrição","Valor (R$)":"Valor"})[["Valor"]],
            st.session_state.df_pix.rename(columns={"Favorecido":"Descrição","Valor (R$)":"Valor"})[["Valor"]]
        ])
